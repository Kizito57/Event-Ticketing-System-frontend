...
const VenueManagement: React.FC = () => {
  ...
  const venueStats = {
    total: venues.length,
    capacity: venues.reduce((sum, v) => sum + (v.capacity || 0), 0),
    active: venues.filter((v) => getVenueStatus(v.venue_id).isActive).length,
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-2">
        {[
          ...
          { label: 'Total Capacity', value: venueStats.capacity.toLocaleString(), icon: Users, color: 'text-green-600' },
                  ...
                  <tr key={venue.venue_id}>
                    <td>
                      <div className="w-20 h-14 rounded overflow-hidden">
                        {venue.image_url ? (
                          <img src={venue.image_url.startsWith('http') ? venue.image_url : `${API_BASE_URL}${venue.image_url}`} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full bg-base-200 flex items-center justify-center text-sm text-base-content/50">No Image</div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="flex flex-wrap gap-2">
                      <button onClick={() => handleViewEvents(venue)} className="btn btn-sm btn-ghost" title="View Events">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleEdit(venue)} className="btn btn-sm btn-info">Edit</button>
                      <button onClick={() => handleDelete(venue)} className="btn btn-sm btn-error">Delete</button>
                    </td>
                  </tr>
                )
              })
          ...
          <div className="card-body">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="form-control">
                    <label className="label"><span className="label-text">Venue Name</span></label>
                  ...
                  <div className="form-control">
                    ...
                    <textarea className="textarea textarea-bordered" rows={3} value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} required />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text">Capacity</span></label>
                    <input type="number" min="1" className="input input-bordered" value={formData.capacity} onChange={(e) => setFormData({ ...formData, capacity: e.target.value })} required />
                  </div>
                </div>

                <div className="space-y-4">
                  <div className="form-control">
                    <label className="label"><span className="label-text">Venue Image</span></label>
                    <input type="file" accept="image/*" onChange={handleImageChange} className="file-input file-input-bordered w-full" />
                  </div>
                  <div className="w-full">
                    <label className="label"><span className="label-text">Preview</span></label>
                    <div className="w-full h-48 border-2 border-dashed border-base-300 rounded-lg overflow-hidden">
                      {imagePreview ? (
                        <img src={imagePreview.startsWith('data:') ? imagePreview : `${API_BASE_URL}${imagePreview}`} className="w-full h-full object-cover" />
                      ) : (
              ...
              <div className="card-actions justify-end mt-6">
                ...
                <button type="submit" className="btn btn-primary">{editingVenue ? 'Update Venue' : 'Add Venue'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEventModal && viewingVenue && (
        <div className="fixed inset-0 z-51 bg-white/50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral text-base-content max-w-2xl w-full p-6 rounded-lg shadow-xl relative border border-base-300">
            ...
            <button onClick={() => setShowEventModal(false)} className="btn btn-sm btn-circle absolute top-4 right-4">✕</button>
